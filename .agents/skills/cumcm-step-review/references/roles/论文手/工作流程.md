...
用户要求的全部格式冻结且各自确定性门禁全部返回 0 后，按 `<SKILL_ROOT>/references/Subagent调度.md` 派发独立质检 Subagent。质检者核对当届规则、主张—证据、数值与单位、图表编号和正文引用、文献及实际渲染效果；同时生成两种格式时再检查 Word/LaTeX 一致性。失败回执必须定位到页码、章节、命令或来源，并按根因返回对应角色；`W2` 未返回 `PASS` 不得交付。

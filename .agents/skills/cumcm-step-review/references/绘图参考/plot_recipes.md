...
### 8c. ggpairs 式相关性矩阵（下三角散点 + 上三角相关系数 + 对角线分布）

**画法来源**：公众号文章《什么，多变量相关性分析还能这样来？》（R 的 `GGally::ggpairs`，2026-08 学习入库）。要点：

- **下三角**：半透明散点（`alpha=0.5`），避免过绘制；
- **上三角**：标注两两相关系数 `r`，可强调色区分强弱；
- **对角线**：分布（直方图/KDE）；
- **分组时**：颜色 + 形状**双通道编码**（`color` 与 `shape` 同时映射分组），不依赖单一颜色通道（色盲友好、冗余编码）；调色板用技能期刊色板（默认 Okabe-Ito）。
- 变量数 3–8 用本画法；**> 8 改用 §8a 相关性热力图**。

```python
# ggpairs 式相关性矩阵（Python/matplotlib 等价实现）
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
df['group'] = rng.choice(['Ctrl', 'Treat'], n)

cols = ['feature_A', 'feature_B', 'feature_C', 'feature_D']
data = df[cols]
groups = sorted(df['group'].unique())
# 色盲安全 + 形状双通道（Okabe-Ito 蓝/朱红 + 圆/方）
PAL = {'Ctrl': '#0072B2', 'Treat': '#D55E00'}
SHP = {'Ctrl': 'o', 'Treat': 's'}

fig, axes = plt.subplots(len(cols), len(cols), figsize=(6.0, 6.0))
for i, yi in enumerate(cols):
    for j, xj in enumerate(cols):
        ax = axes[i, j]
        if i == j:                                   # 对角线：分布
            for g in groups:
                vals = df.loc[df['group'] == g, yi]
                ax.hist(vals, bins=20, alpha=0.4, density=True, color=PAL[g])
            ax.tick_params(labelleft=False, labelbottom=False)
        elif i > j:                                  # 下三角：半透明散点
            for g in groups:
                sub = df[df['group'] == g]
                ax.scatter(sub[xj], sub[yi], s=10, alpha=0.5,
                           color=PAL[g], marker=SHP[g], edgecolor='none')
            ax.tick_params(labelleft=False, labelbottom=False)
        else:                                        # 上三角：相关系数
            r = data[[xj, yi]].corr().iloc[0, 1]
            ax.text(0.5, 0.5, f"r={r:.2f}", ha='center', va='center',
                    fontsize=8.5,
                    color='#D55E00' if abs(r) > 0.5 else '#7F8C8D')
...
fig.tight_layout()
# 图内不写标题（用户定制），信息放图注
export_figure(fig, 'figs/08c_ggpairs_corr_matrix', formats=['png'],
              size_inches=(6.0, 6.0), dpi=300)
```

> R 原版（来源文章）供对照：`GGally::ggpairs(env, columns = c(2:7), lower = list(continuous = wrap("points", alpha = 0.5, color = "blue")), upper = list(continuous = wrap("cor", size = 5, color = "red")))`；分组时 `mapping = aes(color = glt, shape = glt)` + `scale_color_manual()` + `scale_shape_manual()`。
